...
static void sensing_radiation(void* ptr){
    // Publish something
     ...
     printf("Publishing to topic %s!\n",sub_topic);
      ...
      ctimer_set(&sensing_radiation_timer, 10 * CLOCK_SECOND, sensing_radiation,NULL);
}

/*---------------------------------------------------------------------------*/
PROCESS(mqtt_client_radiation, "MQTT sensor radiation");
AUTOSTART_PROCESSES(&mqtt_client_radiation);
            ...
            uint16_t chunk_len)
{
    ...
    LOG_INFO("Radiation Pub Handler: topic=RADIATION, min=%d max=%d\n", min_radiation_parameter, max_radiation_parameter);
}
/*---------------------------------------------------------------------------*/
static void
mqtt_event(struct mqtt_connection *m, mqtt_event_t event, void *data)
{
  ...
  case MQTT_EVENT_DISCONNECTED: {
    ...
    process_poll(&mqtt_client_radiation);
    break;
  }
...
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(mqtt_client_radiation, ev, data)
{
  ...
  // Broker registration					 
  mqtt_register(&conn,&mqtt_client_radiation, client_id, mqtt_event,
  ...
  while(1) {
    ...
    if((ev == PROCESS_EVENT_TIMER && data == &periodic_timer) || 
		  ...
		  if(state==STATE_CONNECTED){
			  ...
			  strcpy(sub_topic,"actuator_radiation");  //YOU DA CAMBIARE STRINGA PER ATTUATORE
      ...
      if(first_publication==1){
          ctimer_set(&sensing_radiation_timer, SENSOR_TIME * CLOCK_SECOND, sensing_radiation,NULL);
          first_publication=0;
      }
