...
static struct ctimer sensing_em_timer;
...
static void sensing_em(void* ptr){
    // Publish something
     ...
     printf("Publishing to topic %s!\n",sub_topic);
      ...
      ctimer_set(&sensing_em_timer, 10 * CLOCK_SECOND, sensing_em,NULL);
}

/*---------------------------------------------------------------------------*/
PROCESS(mqtt_client_em, "MQTT sensor em");
AUTOSTART_PROCESSES(&mqtt_client_em);
            ...
            uint16_t chunk_len)
{
    ...
    LOG_INFO("Electromagnetic Pub Handler: topic=ELECTROMAGNETIC, min=%d max=%d\n", min_em_parameter, max_em_parameter);
}
/*---------------------------------------------------------------------------*/
static void
...
/*---------------------------------------------------------------------------*/
PROCESS_THREAD(mqtt_client_em, ev, data)
{
  ...
  // Broker registration					 
  mqtt_register(&conn,&mqtt_client_em, client_id, mqtt_event,
  ...
  while(1) {
    ...
    if((ev == PROCESS_EVENT_TIMER && data == &periodic_timer) || 
		  ...
		  if(state==STATE_CONNECTED){
			  ...
			  strcpy(sub_topic,"actuator_em");  //YOU DA CAMBIARE STRINGA PER ATTUATORE
      ...
      if(first_publication==1){
          ctimer_set(&sensing_em_timer, SENSOR_TIME * CLOCK_SECOND, sensing_em,NULL);
          first_publication=0;
      }
